// JavaScript code for SPCF Digital Workflow Management System
        ...
        // Initialize interface - always start with login
        document.addEventListener('DOMContentLoaded', function() {
            // Clear any saved login data to ensure fresh start
        ...
        function selectLoginType(type) {
            ...
            // Reset button styles
            employeeBtn.className = 'btn btn-outline-primary flex-fill';
            studentBtn.className = 'btn btn-outline-success flex-fill';
            adminBtn.className = 'btn btn-outline-danger w-100';
            
            // Highlight selected type
            if (type === 'employee') {
                employeeBtn.className = 'btn btn-primary flex-fill';
                loginButton.innerHTML = '<i class="bi bi-briefcase me-2"></i>Sign In as Employee';
                loginButton.className = 'btn btn-primary w-100 btn-lg mb-3';
                loginButton.disabled = false;
            } else if (type === 'student') {
                studentBtn.className = 'btn btn-success flex-fill';
                loginButton.innerHTML = '<i class="bi bi-mortarboard me-2"></i>Sign In as Student';
                loginButton.className = 'btn btn-success w-100 btn-lg mb-3';
                loginButton.disabled = false;
            } else if (type === 'admin') {
                adminBtn.className = 'btn btn-danger w-100';
                loginButton.innerHTML = '<i class="bi bi-shield-lock me-2"></i>Sign In as Administrator';
                loginButton.className = 'btn btn-danger w-100 btn-lg mb-3';
                loginButton.disabled = false;
            }
        }
        ...
        document.getElementById('loginForm').addEventListener('submit', function(e) {
            ...
            if (!selectedLoginType) {
                showLoginError('Please select whether you are logging in as an Employee or Student.');
                return;
            }
            ...
            if (users[userId] && users[userId].password === password) {
                ...
                if (users[userId].role === selectedLoginType) {
                    ...
                    addAuditLog('LOGIN', 'Authentication', `User ${currentUser.firstName} ${currentUser.lastName} logged in successfully`, userId, 'User', 'INFO');
                    
                    showDashboard();
                } else {
                    const actualType = users[userId].role === 'employee' ? 'Employee' : 'Student';
                    const selectedTypeDisplay = selectedLoginType === 'employee' ? 'Employee' : 'Student';
                    ...
                    // Add audit log entry for role mismatch
                    addAuditLog('LOGIN_FAILED', 'Security', `Login attempt with role mismatch for user: ${userId}`, userId, 'User', 'WARNING');
                }
            } else if (!users[userId]) {
                showLoginError('User ID not found. Please check your credentials and try again.');
                
                // Add audit log entry for invalid user
                addAuditLog('LOGIN_FAILED', 'Security', `Login attempt with invalid user ID: ${userId}`, userId, 'User', 'WARNING');
            } else {
                showLoginError('Incorrect password. Please try again.');
                
                // Add audit log entry for wrong password
                addAuditLog('LOGIN_FAILED', 'Security', `Login attempt with incorrect password for user: ${userId}`, userId, 'User', 'WARNING');
            }
        });
        ...
        function showNotifications() {
            if (currentUser && currentUser.role === 'student') {
                alert('📢 Notifications - Student View\n\n' +
                      '🔔 Recent Updates:\n' +
                      '• Engineering Symposium - Tomorrow 9:00 AM\n' +
                      '• Document submission approved ✅\n' +
                      '• New event: Business Plan Presentation\n' +
                      '• Reminder: Club meeting today 3:00 PM\n' +
                      '• System maintenance scheduled\n\n' +
                      'This would open your notification center');
            } else {
